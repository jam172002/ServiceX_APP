...
class BookingRepository {
  ...
  Stream<List<BookingModel>> watchUserBookings(String userId) {
    return _col.where('userId', isEqualTo: userId).orderBy('startTime', descending: true).snapshots().map((q) {
      return q.docs.map((d) {
        final data = d.data();
        data['id'] = d.id;
  ...
  Stream<List<BookingModel>> watchProviderBookings(String providerId) {
    return _col.where('providerId', isEqualTo: providerId).orderBy('startTime', descending: true).snapshots().map((q) {
      return q.docs.map((d) {
        final data = d.data();
        data['id'] = d.id;
        return BookingModel.fromJson(data);
