...
class JobRepository {
  ...
  JobRepository({FirebaseFirestore? db}) : _db = db ?? FirebaseFirestore.instance;
  ...
  Stream<List<JobRequestModel>> watchUserJobs(String userId) {
    return _col.where('userId', isEqualTo: userId).orderBy('createdAt', descending: true).snapshots().map((q) {
      return q.docs.map((d) {
        final data = d.data();
        data['id'] = d.id;
        return JobRequestModel.fromJson(data);
  ...
  Stream<List<JobRequestModel>> watchProviderJobs(String providerId) {
    return _col.where('providerId', isEqualTo: providerId).orderBy('createdAt', descending: true).snapshots().map((q) {
      return q.docs.map((d) {
        final data = d.data();
        data['id'] = d.id;
        return JobRequestModel.fromJson(data);
  ...
  Stream<List<JobRequestModel>> watchOpenJobs() {
    return _col.where('providerId', isNull: true).orderBy('createdAt', descending: true).snapshots().map((q) {
      return q.docs.map((d) {
        final data = d.data();
        data['id'] = d.id;
        return JobRequestModel.fromJson(data);
  ...
  Future<void> updateStatus(String jobId, JobStatus status) async {
    await _col.doc(jobId).set({'status': enumToString(status)}, SetOptions(merge: true));
  }
  ...
  Future<void> assignProvider(String jobId, String providerId) async {
    await _col.doc(jobId).set({'providerId': providerId, 'status': enumToString(JobStatus.accepted)}, SetOptions(merge: true));
  }
}
