...
class ProviderRepository {
  ...
  ProviderRepository({FirebaseFirestore? db}) : _db = db ?? FirebaseFirestore.instance;
  ...
  Future<void> upsertProviderProfile(ProviderProfileModel profile) async {
    await _doc(profile.providerId).set(profile.toJson(), SetOptions(merge: true));
  }
  ...
  Future<void> setAvailability(String providerId, bool isAvailable) async {
    await _doc(providerId).set({'isAvailable': isAvailable}, SetOptions(merge: true));
  }
}
