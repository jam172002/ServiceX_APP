...
class WalletRepository {
  ...
  Future<String> addTransaction({
    ...
    await _db.runTransaction((t) async {
      ...
      final data = snap.data() ?? <String, dynamic>{};
      final current = (data['balance'] is num) ? (data['balance'] as num).toDouble() : 0.0;
      final next = type == TransactionType.credit ? current + amount : current - amount;
  ...
  Stream<List<TransactionModel>> watchTransactions(String walletId, {int limit = 200}) {
    return _txCol
  ...
  Future<String> addPaymentCard({
    ...
    final card = PaymentCardModel(id: ref.id, userId: userId, brand: brand, last4: last4, isDefault: makeDefault);

    await _db.runTransaction((t) async {
      if (makeDefault) {
        final existing = await _cardCol.where('userId', isEqualTo: userId).get();
  ...
  Future<void> setDefaultCard(String userId, String cardId) async {
    await _db.runTransaction((t) async {
      ...
      for (final d in all.docs) {
        t.set(d.reference, {'isDefault': d.id == cardId}, SetOptions(merge: true));
      }
    });
  }
