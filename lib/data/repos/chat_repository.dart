...
class ChatRepository {
  ...
  Stream<List<ChatRoomModel>> watchUserRooms(String userId) {
    return _rooms.where('userId', isEqualTo: userId).orderBy('lastMessageTime', descending: true).snapshots().map((q) {
      return q.docs.map((d) {
        final data = d.data();
        data['id'] = d.id;
  ...
  Stream<List<ChatRoomModel>> watchProviderRooms(String providerId) {
    return _rooms.where('providerId', isEqualTo: providerId).orderBy('lastMessageTime', descending: true).snapshots().map((q) {
      return q.docs.map((d) {
        final data = d.data();
        data['id'] = d.id;
        return ChatRoomModel.fromJson(data);
  ...
  Future<void> sendMessage({
    ...
    final msgRef = _db.collection(FirestorePaths.chatRoomMessagesCol(roomId)).doc(
      message.id.isEmpty ? null : message.id,
    );
    final ref = message.id.isEmpty
        ? _db.collection(FirestorePaths.chatRoomMessagesCol(roomId)).doc()
        : _db.collection(FirestorePaths.chatRoomMessagesCol(roomId)).doc(message.id);
    ...
    await _rooms.doc(roomId).set({'lastMessageTime': Timestamp.fromDate(msg.sentAt)}, SetOptions(merge: true));
  }
  ...
  Stream<List<MessageModel>> watchMessages(String roomId, {int limit = 100}) {
    ...
    return col.orderBy('sentAt', descending: true).limit(limit).snapshots().map((q) {
      return q.docs.map((d) {
        final data = d.data();
        data['id'] = d.id;
